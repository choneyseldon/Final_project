...
class PostService {
    ...
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        if (likedBy.contains(userId)) {
          ...
          likes = likes - 1;
          wasLiked = true; // It was liked, now unliking
        } else {
          likedBy.add(userId);
          likes = likes + 1;
          wasLiked = false; // It wasn't liked, now liking
        }
      ...
      // Create notification if user just liked the post (not unliked)
      if (!wasLiked && postOwnerId.isNotEmpty && postOwnerId != userId) {
  ...
  Future<void> toggleSavePost(String postId, String userId) async {
    try {
      ...
      // First get post information for notification
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        if (savedPostIds.contains(postId)) {
          ...
          wasSaved = true; // It was saved, now unsaving
        } else {
          savedPostIds.add(postId);
          wasSaved = false; // It wasn't saved, now saving
        }
      ...
      // Create notification if user just saved the post (not unsaved)
      if (!wasSaved && postOwnerId.isNotEmpty && postOwnerId != userId) {
        final notificationService = NotificationService();
